...
read_rss <- function(url) {
  ...
  error = function(e) {
    message("Could not read RSS feed, make sure the URL is valid.")
  })
}

extract_data <- function(rss_df) {
  ...
  tryCatch({
    # The item_description column holds data for each earthquake in string format
    description <- rss_df |>
      select(item_description)
  ...
  error = function(e) {
    message("Could not extract earthquake data successfully.")
  })
}
